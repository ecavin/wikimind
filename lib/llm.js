...
function normalizeClusterResponse(raw) {
  const out = { clusters: [], relationships: [], superClusters: [] };
  if (!raw) return out;
  ...
  if (Array.isArray(raw.relationships)) {
    out.relationships = raw.relationships
      .filter((r) => r && r.from && r.to && r.from !== r.to)
      .map((r) => ({
        from: r.from,
        to: r.to,
        description: truncateWords(r.description || '', 6)
      }))
...
function mergeClusterBatches(batches) {
  ...
  for (const batch of batches) {
    ...
    for (const r of batch.relationships || []) rels.push(r);
  }
  return {
...
const CLUSTER_SYSTEM_BASE =
  ...
  '(2) Identify meaningful relationships between article pairs, especially within the same cluster. ' +
  'Include non-obvious conceptual connections — you are NOT limited to explicit Wikipedia links. ' +
  'Each relationship has a direction (from → to) and a very concise description (≤ 6 words). ' +
  'Example: from="China", to="Communist Party of China", description="Party currently in power". ' +
...
async function clusterArticles(articles, apiKey, opts = {}) {
  ...
  if (prior && Array.isArray(prior.clusteredTitles)) {
    ...
    const system = CLUSTER_SYSTEM_BASE + ' ' +
      ...
      'structure, and relationships unless clearly wrong. Place the NEW articles below into existing ' +
      'clusters when they fit; create new small clusters only when necessary. Add relationships for ' +
      'newly relevant connections (including between new and prior articles). Preserve the ' +
      'super-cluster tree.';
    const priorPayload = {
    ...
    const user =
      ...
      'Return the UPDATED taxonomy as JSON: {clusters, relationships, superClusters}. ' +
