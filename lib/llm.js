...
const GEMINI_FALLBACK_ENDPOINT = `${GEMINI_BASE}/${GEMINI_FALLBACK_MODEL}:generateContent`;
...
async function callGeminiEndpoint(endpoint, safeKey, body) {
...
async function callGemini({ apiKey, system, user, schema, thinkingLevel = 'low' }) {
  ...
  let res = await callGeminiEndpoint(GEMINI_ENDPOINT, safeKey, body);
  if (res.status === 503) {
    res = await callGeminiEndpoint(GEMINI_FALLBACK_ENDPOINT, safeKey, body);
  }
  if (!res.ok) {
